...
public class Main_15644_구슬탈출3 {
	...
	static class Pos {
		...
		public Pos(Ball red, Ball blue, int time, String method) {
			this.red = red;
			this.blue = blue;
			this.time = time;
			this.method = method;
	...
	public static void main(String[] args) throws IOException {
		...
		Pos answer = bfs(redBall, blueBall);
		System.out.print(answer.time == -1 ? -1 : answer.time + "\n" + answer.method);
	}

	public static Pos bfs(Ball redBall, Ball blueBall) {
		...
		while(!que.isEmpty()){
			...
			for(int d = 0; d < 4; d++){
				...
				if(blueOut) continue;
				else if(redOut) return new Pos(nextRed, nextBlue, curr.time + 1, curr.method + direction[d]);
				...
				que.offer(new Pos(nextRed, nextBlue, curr.time + 1, curr.method + direction[d]));
			}
		}

		return new Pos(null, null, -1, "");
	}
